function Verifica() {
    ...
    if (!user || !senha) {
        ...
        window.location.href = "Inscrição.html";
    }
}
...
function crialista() {
    ...
    for (let i = 0; i < Camisas.length; i++) {
        tabela += "<tr><td>" + Nome[i] + "</td><td>" + Camisas[i] + "</td><td>" + CPF[i] + "</td><td>" + Genero[i] + "</td><td><button class='btn btn-success' onclick='editar(" + i + ")'>Editar</button><button class='btn btn-danger' onclick='excluir(" + i + ")'>Excluir</button></td></tr>";
    }
